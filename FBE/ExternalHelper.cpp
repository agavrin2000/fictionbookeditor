...
/// <summary>Fill visible elements structure</summary>
...
/// <summary>Construct popup menu of visible elements</summary>
...
static CMenu MakeDescComponentsMenu()
{
	...
	for (int i = 0; i < g_desc_elements.GetSize(); ++i)
	{
		if (g_desc_elements.GetValueAt(i).groupid == 0)
		{
			...
			cur.CheckMenuItem(i + MENU_BASE, ext ? MF_CHECKED : MF_UNCHECKED);
			continue;
		}
...
/// <summary>Popup visible elements menu. Call from Script</summary>
/// <returns>element_id=Menu Item</summary>
...
static CSimpleArray<CString> g_genre_groups;    // genre groups
static CSimpleArray<Genre> g_genres;            // genres

/// <summary>Load genre list from file</summary>
...
/// <summary>Construct genres popup menu</summary>
/// <returns>Menu</summary>
...
/// <summary>Popup genres menu. Call from Script</summary>
/// <returns>name=Menu Item</summary>
	...
	if(popup)
	{
		...
		if(cmd < (UINT)g_genres.GetSize())
		{
			*name = g_genres[cmd].id.AllocSysString();
			::SendMessage(::GetActiveWindow(), WM_COMMAND, MAKELONG(0, IDN_ED_CHANGED), 0);
			//::PostMessage(::GetActiveWindow(), WM_COMMAND, MAKELONG(ID_DOC_CHANGED, 0), 0);
			return S_OK;
		}
	}
	*name = NULL;
...
static CSimpleArray<CString> g_lang_groups; // language groups: European, Asian, etc.
static CSimpleArray<Lang> g_langs;          // languages

/// <summary>Construct languages dropdown menu</summary>
/// <returns>Menu</summary>
